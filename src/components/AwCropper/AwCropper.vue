...
function cropperModule(props: AwCropperProps, preview: Preview, emit: Emit) {
  ...
  const setCropStyle = () => {
    ...
    if (cropShape === "circle") {
      ...
      (el.querySelector(
        ".cropper-face"
      )! as HTMLElement).style.backgroundColor = "unset";
      Object.entries(styles).forEach(([name, value]: [string, string]) => {
        (crop.style as any)[name] = value;
      });
    } else {
      if (
        typeof options.cropBoxResizable === "boolean" &&
        !options.cropBoxResizable
      ) {
        el.style.opacity = "0";
      }
    }
  ...
  const getImageData = (options: CroppedCanvasOptions, type = "Blob") => {
    return new Promise((resolve, reject) => {
      if (!cropper.isInit) {
        ...
        cropper
          .instance!.getCroppedCanvas(options)
